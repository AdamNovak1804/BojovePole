<template>
    <form action="POST" @submit.prevent="">
        <b-modal
            size="xl"
            ref="cemetery-edit-view"
            hide-footer
            @shown="reloadSize()"
        >
            <h1 class="text-center">Návrh o pridaní cintorína</h1>
            <add-map 
                ref="cemetery-edit-map"
                v-on:changeMarker="updateLatLng"
            />
            <b-row no-gutters class="mt-3">
                <b-col col lg="3">
                    <div>
                        <label class="label-form" for="cemetery-edit-name">
                            Názov cintorína
                        </label>
                        <input v-model="form.name" type="text" name="cemetery-edit-name" id="cemetery-edit-name">
                    </div>
                </b-col>
                <b-col>
                    <label class="label-form" for="cemetery-edit-description">
                        Opis a história cintorína
                    </label>
                    <textarea 
                        v-model="form.description"
                        class="textarea"
                        name="cemetery-edit-description"
                        id="cemetery-edit-description"
                        rows="10"
                    />
                    <b-row>
                        <b-col md="8">
...
<script>
    ...
    export default {
        ...
        methods: {
            showModal: function() {
                this.form.id = this.cemetery.id;
                this.form.name = this.cemetery.name;
                this.form.description = this.cemetery.description;
                this.form.position = [ 
                    this.cemetery.longtitude, 
                    this.cemetery.latitude
                ];

                if ( !!this.cemetery.gallery ) {
                    this.form.gallery = JSON.parse(this.cemetery.gallery);
                }
                else {
                    this.form.gallery = { images: '' };
                }

                this.$refs['cemetery-edit-view'].show();
            },
            ...
            saveRequest: function() {
                ...
                axios.post('/api/update_cemetery', form).then(() => {
            ...
            deleteRequest: function() {
                axios.delete('/api/delete_cemetery', { data: this.form }).then(() => {
            ...
            reloadSize: function() {
                this.$refs['cemetery-edit-map'].refreshSize();
            },
